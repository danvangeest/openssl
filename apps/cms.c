...
const OPTIONS cms_options[] = {
    ...
    {"recip_ukm", OPT_RECIP_UKM, '<', "KEMRecipientInfo user keying material for current recipient"},
 ...
 opthelp:
    ...
    if (recip_first != NULL) {
        ...
        for (ropt = recip_first; ropt; ropt = ropt->next) {
            if (ropt->ukm_file != NULL) {
                BIO *kemri_ukmbio = BIO_new_file(ropt->ukm_file, "rb");

                if (kemri_ukmbio == NULL) {
                    BIO_printf(bio_err, "Can't open UKM file %s\n", ropt->ukm_file);
                    goto end;
                }
                ropt->ukm_data_length = bio_to_mem(&ropt->ukm_data, -1, kemri_ukmbio);
                BIO_free(kemri_ukmbio);
                if (ropt->ukm_data_length < 0) {
                    BIO_printf(bio_err, "Error reading UKM data\n");
                    goto end;
                }
            }
        }
    ...
    for (recip_opt = recip_first; recip_opt;) {
        cms_recip_opt *topt;
