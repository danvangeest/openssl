...
static int kem_cms_encrypt(CMS_RecipientInfo *ri)
{
    ...
    if (kdf_obj == NULL || OBJ_obj2nid(kdf_obj) == NID_undef) {
        /*
         * TODO: OPENSSL MAINTAINERS: options:
         *   - query provider for default KDF for this key?
         *   - have a global default if the user didn't provider a KDF?
         *   - error if the user didn't provider a KDF?
         */
        /*
        ...
        if (OSSL_PARAM_modified(&params[0])) {
            ...
            x509_algor = d2i_X509_ALGOR(NULL, &p, params[0].return_size);
            if (x509_algor == NULL)
                goto err;
            if (!X509_ALGOR_copy(kdf, x509_algor))
                goto err;
        } else {
            /* TODO: OPENSSL MAINTAINERS: set a global default KEMRI KDF, or fail? */
