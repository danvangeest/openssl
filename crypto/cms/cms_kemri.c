                          ...
                          CMS_KEMRecipientInfo *kemri, int enc)
{
    ...
    if (!EVP_CipherUpdate(kemri->ctx, NULL, &outlen, in, inlen))
        goto err;
    out = OPENSSL_malloc(outlen);
    if (out == NULL)
        goto err;
    if (!EVP_CipherUpdate(kemri->ctx, out, &outlen, in, inlen))
        goto err;
    *pout = out;
    out = NULL;
                                        ...
                                        CMS_RecipientInfo *ri)
{
    ...
    ASN1_STRING_set0(kemri->kemct, kem_ct, kem_ct_len);
    kem_ct = NULL;

    if (!cms_kek_cipher(&enckey, &enckeylen, kem_secret, kem_secret_len, ec->key, ec->keylen,
                        kemri, 1))
        goto err;
    ASN1_STRING_set0(kemri->encryptedKey, enckey, enckeylen);
